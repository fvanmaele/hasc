...
inline void transpose1(int n, double *A, double *B)
{
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++)
      B[i * n + j] = A[j * n + i];
...
inline void transpose2(int n, double *A, double *B)
{
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++)
      B[j * n + i] = A[i * n + j];
...
inline void transpose3(int n, double *A, double *B)
{
  if (n % M != 0)
  {
...
template <int M, int N> // MxN blocks
inline void transpose4(int n, double *A, double *B)
{
  if (n % M != 0)
  {
    std::cout << n << " is not a multiple of " << M << std::endl;
...
inline void transpose5(int n, int b, double *A, double *B)
{
  if (b % M != 0)
  {
...
// assume A and B are NOT the same matrix
template <int M> // bxb blocks with M blocking in the strided direction
inline void transpose6(int n, int b, double *A, double *B)
{
  if (b % M != 0)
  {
    std::cout << b << " is not a multiple of " << M << std::endl;
